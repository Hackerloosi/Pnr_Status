...
bot.onText(/\/ban (\d+)/, (msg, match) => {
  ...
  bot.sendMessage(userId, "🚫 You are banned from this bot.");
});
...
bot.onText(/^\d{10}$/, async (msg) => {
  ...
  if (getStatus(userId) !== "APPROVED") return;

  const pnr = msg.text;

  try {
    const res = await fetch("https://api.v2.pnr.railcore.tech/api/v1/pnr/status", {
      method: "POST",
    ...
    if (!json.success) {
      return bot.sendMessage(chatId, "❌ Invalid PNR");
    }
...
\`\`\``;
    ...
    bot.sendMessage(chatId, "⚠️ Service unavailable");
  }
});